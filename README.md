...
- 当前桌面端和内置 DSH 均为 `0.1.5-rc.1`。对 `0.1.5-alpha.1`、`0.1.5-alpha.2` 和 `0.1.5-rc.1` 的旧日志迁移，启动器仅为已审计的 `dfy-media` 图片块和 `dfy-session-image` 生成图片块补充结构校验及准入，保留原内容和资源引用。事件转换、原日志保留和新版日志发布仍由 DSH 执行；未知块及未知字段继续拒绝迁移。侧栏点击不存在的会话时会显示错误提示。

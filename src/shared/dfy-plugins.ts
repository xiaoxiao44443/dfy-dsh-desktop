...
import type { DfyPluginDefinition, ManagedPluginEntry } from './contracts.js'
...
/** A whole bundle takes precedence over its members in one install selection. */
export function normalizeDfySelection(names: string[], entries: DfyPluginDefinition[]): string[] {
  const included = new Set(entries.filter(entry => names.includes(entry.name)).flatMap(entry => entry.includes ?? []))
  return [...new Set(names)].filter(name => !included.has(name))
}

export function includedDfyPlugins(plugins: ManagedPluginEntry[]) {
  return new Map(plugins.flatMap(bundle => (bundle.includedPlugins ?? []).map(member => [member.name, { ...member, bundle }] as const)))
}
...
export function parseDfyPluginCatalog(value: unknown): DfyPluginDefinition[] {
  ...
  const plugins = value.plugins.map((entry: unknown) => {
    ...
    names.add(name)
    if (entry.includes !== undefined && (!Array.isArray(entry.includes) || entry.includes.length === 0 || entry.includes.length > 100
      || new Set(entry.includes).size !== entry.includes.length
      || entry.includes.some(member => typeof member !== 'string' || member === name || !/^@dfy-plugins\/[a-z0-9][a-z0-9._-]*$/u.test(member)))) {
      throw new Error('插件组合包清单无效。')
    }
    ...
    return {
      ...
      ...(entry.includes === undefined ? {} : { includes: entry.includes as string[] }),
    }
  }).filter((entry) => entry.name !== '@dfy-plugins/dsh-vision')
  for (const entry of plugins) {
    if (entry.includes?.some(name => name === '@dfy-plugins/dsh-vision' || plugins.find(member => member.name === name)?.includes !== undefined)) {
      throw new Error('组合包不能包含停更插件或其他组合包。')
    }
  }
  return plugins
}
